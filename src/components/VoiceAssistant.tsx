  ...
  return (
    <>
      <button 
        ...
        className="fixed bottom-8 right-8 w-16 h-16 bg-teal-600 text-white rounded-full shadow-2xl shadow-teal-400 flex items-center justify-center hover:scale-110 transition-transform z-50 group"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="fixed bottom-28 right-8 w-80 bg-slate-900 rounded-[32px] shadow-2xl border border-slate-800 z-50 overflow-hidden"
          >
            <div className="p-6 bg-teal-600 text-white flex items-center justify-between">
              ...
              <button onClick={() => setIsOpen(false)} className="p-1 hover:bg-slate-800/40 rounded-lg transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              <div className="min-h-[100px] flex flex-col justify-center text-center space-y-4">
                {errorMessage ? (
                  <div className="p-4 bg-red-950/40 text-red-600 rounded-2xl text-xs font-medium border border-red-800/60">
                    {errorMessage}
                  </div>
                ) : transcript ? (
                  <div className="space-y-4">
                    <p className="text-sm text-slate-500 font-medium italic">"{transcript}"</p>
                    {response && (
                      <motion.div 
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="p-4 bg-teal-950/40 text-teal-300 rounded-2xl text-sm font-medium leading-relaxed"
                      >
                        {response}
                      </motion.div>
                    )}
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed">
                    ...
                    <span className="font-bold text-teal-600">"Show my medicines"</span>
                  </p>
                )}
              </div>
              ...
              <div className="flex justify-center">
                <button 
                  ...
                  className={cn(
                    ...
                    isListening ? "bg-red-500 text-white animate-pulse scale-110" : "bg-teal-900/45 text-teal-600 hover:bg-teal-800/60"
                  )}
                >
...
function VoiceHint({ icon, label }: { icon: React.ReactNode, label: string }) {
  return (
    <div className="flex flex-col items-center gap-1 p-2 rounded-xl bg-slate-900 border border-slate-800">
