...
export default function SignupPage({ onSignup }: { onSignup: (user: UserProfile) => void }) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (errorCode === 'auth/email-already-in-use' || errString.includes('auth/email-already-in-use')) {
      ...
      } else if (errorCode === 'auth/weak-password' || errString.includes('auth/weak-password')) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4">
      <div className="max-w-md w-full bg-slate-900 rounded-3xl shadow-xl shadow-slate-200 overflow-hidden">
        <div className="p-8 md:p-12">
          ...
          <h2 className="text-2xl font-bold text-slate-50 mb-2 text-center">Create Account</h2>
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-4 py-3 bg-slate-900 border border-slate-700/70 rounded-2xl focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-200 mb-1">Caregiver Phone Number (for alerts)</label>
                <div className="relative">
                  <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={20} />
                  <input 
                    type="tel" 
              ...
              <button 
                type="submit" 
                disabled={loading}
                className="w-full py-4 bg-teal-600 text-white rounded-2xl font-semibold hover:bg-teal-700 transition-colors shadow-lg shadow-teal-200 flex items-center justify-center gap-2 mt-4 disabled:opacity-50"
              >
